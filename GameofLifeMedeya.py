...
class Field:
  ...
  def __str__(self):
    for i in range (25, self.w + 25):
      for j in range(25, self.l + 25):
...
class Civilian(Cell):
  def __init__(self, a1, a2, l, w ,n1, n2, n3, n4, n5, n6, n7, n8, n9, nCurrent):
      ...
      for k in self.array:
        i, j = k; x, y = n9
        self.a2[x][y] = 0 if self.a1[i][j] == "М" and (__import__('random').random()) >= 0.25 or [self.a1[i[0]][i[1]] for i in self.array].count(0) != 2 or [self.a1[i[0]][i[1]] for i in self.array].count(0) != 3 else "*"
N, M = map(int, input().split()); a1 = [[1]*(N + 50) for g in range(M + 50)]
for i in range(25, M + 25):
  for j in range(25, N + 25):
    a1[i][j] = randomizer()
a2 = [[0]*(N + 50) for g in range(M + 50)]; a = Field(a1, a2, N, M); a.next(); a.__str__(); print(); a.next(); a.__str__(); print(); a.next(); a.__str__(); print()
